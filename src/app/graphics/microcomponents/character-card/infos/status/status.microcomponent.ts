...
@Component({
  selector: 'rem-character-card-infos-status-microcomponent',
  template: '',
  styleUrls: ['./status.microcomponent.scss'],
})
export class CharacterCardInfosStatusMicrocomponent extends TypedTextShape {
  constructor(
    ...
    this.characterCardService.$selectedCharacters.subscribe(
      (lastSelectedCharacter) => {
        this.currentData.next(lastSelectedCharacter?.status?? '');
      }
    );
  }
  ...
  override ngOnInit(): void {
    ...
    this.plottedD3Element?.selection.attr('dy', '95');
  }
}
