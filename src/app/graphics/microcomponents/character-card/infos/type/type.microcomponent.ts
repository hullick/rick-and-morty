...
@Component({
  selector: 'rem-character-card-infos-type-microcomponent',
  template: '',
  styleUrls: ['./type.microcomponent.scss'],
})
export class CharacterCardInfosTypeMicrocomponent extends TypedTextShape {
  constructor(
    ...
    this.characterCardService.$selectedCharacters.subscribe(
      (lastSelectedCharacter) => {
        this.currentData.next(lastSelectedCharacter?.type ?? '');
      }
    );
  }
  ...
  override ngOnInit(): void {
    ...
    this.plottedD3Element?.selection.attr('dy', '135');
  }
}
